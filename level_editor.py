...
class TOPBAR_MT_my_menu(bpy.types.Menu):
    ...
    def draw(self, context):
        ...
        #context : カーソルに合わせた時のポップアップのカスタマイズなどに使用
...
class MYADDON_OT_stretch_vertex(bpy.types.Operator):
    ...
    bl_label = "頂点を伸ばす"
    bl_discription = "頂点座標を引っ張って伸ばします"
    # リドゥ、アンドゥ可能オプション
...
class MYADDON_OT_export_scene(bpy.types.Operator, bpy_extras.io_utils.ExportHelper):
    ...
    def export(self):
        ...
        print("シーンの情報の出力開始... %r" % self.filepath)
    ...
    def parse_scene_recursive(self, file, object, level):
        ...
        # ローカルトランフォーム行列から平行移動、回転、スケーリングを抽出
        # 型は VectorQuaternion, Vector
    ...
    def parse_scene_recursive_json(self, data_parent, object, level):
        ...
        # 平行移動、回転、スケールの抽出
        trans, rot, scale = object.matrix_local.decompose()
        # 回転をQuternion から Euler (3軸で回転角) に変換
        rot = rot.to_euler()
        # ラジアンから度数法に変換
        rot.x = math.degrees(rot.x)
        rot.y = math.degrees(rot.y)
...
class MYADDON_OT_add_collider(bpy.types.Operator):
    bl_idname = "myaddon.myddon_ot_add_collider"
    bl_label = "コライダー 追加"
...
class OBJECT_PT_collider(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        if "collider" in context.object:
            ...
            self.layout.prop(context.object, '["collider_size"]', text = "Sixe")
        else:
            # プロパティがなければ、プロパティ追加ボタンを表示
